<script>
    ...
    // Team assignment related variables
    let studentList = [];
    let usedTeams = []; // This will store already assigned team numbers
    let availableTeams = Array.from({ length: 100 }, (_, i) => (i + 1).toString()); // Initially all teams are available
    ...
    // Function to filter out already used team numbers from availableTeams list
    function filterAvailableTeams() {
        availableTeams = availableTeams.filter(team => !usedTeams.includes(team));
    }

    // Function to assign a random team from availableTeams and remove it from the list
    function assignRandomTeam() {
        const randomIndex = Math.floor(Math.random() * availableTeams.length);
        const selectedTeam = availableTeams[randomIndex];
        availableTeams.splice(randomIndex, 1);  // Remove the selected team from the availableTeams list
        console.log(`Assigned team number: ${selectedTeam}`);
        console.log(`Updated available teams: [${availableTeams.join(', ')}]`);
        return selectedTeam;
    }

    async function choosePlayers() {
        // Only choose players from available teams
        const firstTeamMembers = studentList.filter(student => availableTeams.includes(student.team));
        const secondTeamMembers = studentList.filter(student => availableTeams.includes(student.team));

        console.log(firstTeamMembers);
        const min = Math.min(firstTeamMembers.length, secondTeamMembers.length);

        const randomAudio = new Audio("voicebooking-speech.wav");
        ...
        player1team1 = assignRandomTeam();  // Assign team number and remove it from the list
        player1team2 = assignRandomTeam();  // Assign team number and remove it from the list
        player2team1 = assignRandomTeam();  // Assign team number and remove it from the list
        player2team2 = assignRandomTeam();  // Assign team number and remove it from the list

        console.log(`Assigned numbers: ${player1team1}, ${player1team2}, ${player2team1}, ${player2team2}`);
    }

    onMount(async () => {
        // Fetch the student list and already assigned team numbers from the API
        const response = await axios.get('https://api.itech-club.com/list');
        studentList = response.data;

        // Extract used team numbers from the response data (assuming the 'team' field holds the assigned team number)
        usedTeams = studentList.map(student => student.team.toString());

        // Filter available teams by removing the used ones
        filterAvailableTeams();

        // Play the MP3 when the component is mounted
        const startAudio = new Audio("https://drive.google.com/uc?export=download&id=1-5h7Tk3fnt9zyXxuJ4bUNKxMIyVb_9sw");
        startAudio.play().catch(error => console.error("Audio play failed:", error));

        console.log(`The following teams are now taken: [${usedTeams.join(', ')}]`);
        console.log(`Available team numbers: [${availableTeams.join(', ')}]`);
    });
</script>
...
<div class="all">
    <div class="firstTeam" style="background-color: {colorMap.get(player1team1)};">
        <div class="container">
    ...
    <div class="secondTeam" style="background-color: {colorMap.get(player1team2)};">
        <div class="container">
